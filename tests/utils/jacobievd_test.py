...
def _jacobi_evd_test_main(
    ...
    run_func = (
        serial_jacobi_evd
        if is_plain
        else spsim.sim_jax(sim_dict[field], serial_jacobi_evd, static_argnums=(1,))
    )
    ...
    top_k = 5
    rmse = np.sqrt(np.mean((np.abs(eig_vec_np[:top_k]) - np.abs(eig_vec[:top_k])) ** 2))
...
def test_jacobi_evd_plain(setup_test):
    print(" ========= start test jacobi_evd plain =========\n")
    _jacobi_evd_test_pack(setup_test, is_plain=True)
    print(" ========= test evd plain end ========= \n")


def test_jacobi_evd(setup_test):
    print(" ========= start test jacobi_evd =========\n")
    _jacobi_evd_test_pack(setup_test, is_plain=False)
    print(" ========= test evd end ========= \n")

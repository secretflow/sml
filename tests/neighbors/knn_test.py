...
def test_knn():
    ...
    assert predictions_distance == sklearn_predictions.tolist()


def test_knn_sep():
    sim = spsim.Simulator.simple(3, libspu.ProtocolKind.ABY3, libspu.FieldType.FM64)

    def fit(X_train, y_train, knn_model):
        knn_model.fit(X_train, y_train)

        return knn_model

    def predict(X_test, knn_model):
        return knn_model.predict(X_test)
    ...
    X_test = jnp.array([[4, 2], [1, 3]])
    X_test = jnp.array([[1, 2], [2, 3], [3, 4], [5, 1]])
    ...
    # train result
    res = spsim.sim_jax(sim, predict)(X_train, distance_model)
    print(f"train label: {y_train_new}")
    ...
    logging.info(f"sklearn_predictions: {sklearn_predictions.tolist()}")
    logging.info(f"sml_predictions: {predictions_distance}")
